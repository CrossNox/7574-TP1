...
Sin embargo, al leer, lo que puede pasar es que haya líneas incompletas. Originalmente el CSV se escribia con los campos \texttt{timestamp}, \texttt{metric_id}, \texttt{value}. Podemos pensar varios escenarios:
\begin{itemize}
	\item El caso feliz: Tenemos todos los campos y el \texttt{\\n} que termina la línea
	\item Un caso poco feliz, pero poco problemático: tenemos una coma, pero no el valor que sigue
	\item Un caso sutilmente problematico: que el campo \texttt{value} esté truncado!
\begin{itemize}
